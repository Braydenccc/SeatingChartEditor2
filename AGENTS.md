...
- **Write 工具使用规范**：如果你是 Codex，必须严格遵守以下规则
  - **使用场景**：
    - ✅ 仅用于创建新文件或完全重写文件
...
项目已配置 GitHub MCP server，可直接通过 Codex 管理 GitHub Issues、Pull Requests 等。

### GitHub MCP Server 配置

**全局配置位置**：`~/.Codex/settings.json`
...
```json
{
  ...
  "enabledPlugins": {
    "github@Codex-plugins-official": true
  }
}
```
