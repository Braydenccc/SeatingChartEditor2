...
- **选区拖拽 (`useDragPreview`)**: 支持多选座位后整体拖拽移动。通过 `clientToChartLocal` 将鼠标坐标转换为画布本地坐标，再通过 `chartLocalToGrid` 计算目标网格位置。拖拽时会显示半透明预览，松手后执行批量座位交换。触摸端长按拖拽也按起点判断：起点在当前多选内则带动选区，起点不在多选内则只移动单个座位。
