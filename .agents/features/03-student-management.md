...
- **数值属性 (`useStudentAttributes`)**: 学生支持 `numericAttributes`，属性定义由独立 composable 管理。默认内置“身高 cm”和“成绩 分”，用户可新增自定义数值属性。未被规则引用的属性删除后会同步清理所有学生上的对应值；若任意启用或禁用规则仍通过 `attributeId` 引用该属性，领域层会阻止删除并返回结构化规则引用。
...
- **实体删除的规则引用保护**: `deleteStudent()`、`deleteTag()` 和 `deleteAttribute()` 在修改数据前都会查询完整规则树。学生、标签或数值属性只要仍被任意启用或禁用规则显式引用，删除就会返回 `EntityDeletionResult { success: false, reason: 'referenced-by-rules', references }`，且不清理座位、标签值、数值列、选中态或 Undo/Redo。`references` 按规则提供 `ruleId`、描述、谓词和引用路径，名单界面通过 `useUiFeedback` 告知用户应先修改或删除哪些规则。
- **删除学生清理座位与历史**: 通过引用检查后，`deleteStudent()` 会同步清理座位占用并清空 Undo/Redo，避免旧历史恢复已不存在的学生。`setStudentCount()` 缩减人数时只考虑空白学生，并先完整预检是否存在足够的未引用候选；只有能够一次达到目标人数时才原子删除并清理历史，否则保持名单、座位、选中态和 Undo/Redo 原样，返回 `false`，并通过 `lastStudentDeletionResult` 暴露阻止规则。名单界面在实际缩减前还会要求确认。`clearAllStudents()` 是工作区整体替换/重置使用的内部清理路径，不用于绕过交互式单实体删除保护。
...
- **选中态与规则引用悬空**: 如果编写批量删除学生、标签或属性的组件，应复用各自 composable 的删除入口并处理 `EntityDeletionResult`，不能先清理关联数据再尝试删除，也不能直接改写响应式数组绕过选中态、座位占用和规则引用保护。
