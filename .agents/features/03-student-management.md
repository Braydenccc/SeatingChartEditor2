...
- **本地代理边界**: `server.cjs` 与 `vite.mock.plugin.js` 都实现了 `/api/fuckseats-proxy`，仅允许 GET 到本机常用端口，避免浏览器 CORS 失败，也避免任意 URL 代理。
