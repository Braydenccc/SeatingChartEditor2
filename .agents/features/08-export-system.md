...
- 画布渲染引擎: `src/composables/useImageExport.ts`
- 专属设定表单数据: `src/composables/useExportSettings.ts`
- 文件保存适配层: `src/platform/files.ts`
...
- **不要绕过平台保存层**: 新增导出入口时不要直接写 `a.download` 或 `XLSX.writeFile`，否则 Tauri 桌面端无法使用原生保存路径。
