...
- **全平台兼容警示**: 项目需要编译为浏览器 Web 和 Tauri 桌面端。因此，绝对不要在 `src/components/` 或 `src/composables/` 内部直接调用 Node.js 内置模块（如 `fs`, `path`）。如果必须访问本地能力，应通过 Tauri 插件、Tauri command 或 WebDAV 的形式隔离。
