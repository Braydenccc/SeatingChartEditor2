...
test('enters, assigns, undoes, saves, reopens and reaches export', async ({ page }, testInfo) => {
  ...
  const findCandidate = async () => {
    ...
    await page
      .locator('.context-rail .n-tabs-tab:visible')
      .filter({ hasText: /^学生$/ })
      .click()
    return page.locator('.student-pool-panel:visible .candidate-item').filter({ hasText: studentName })
  }
  ...
  const nameInput = page.locator('input[placeholder="未命名"]:visible').first()
  await nameInput.pressSequentially(studentName)
  await nameInput.press('Tab')
  await expect(nameInput).toHaveValue(studentName)
  ...
  const firstAvailableSeat = page
    .locator('[data-seat-id][aria-disabled="false"]')
    .filter({ hasText: '空位' })
    .first()
  const targetSeatId = await firstAvailableSeat.getAttribute('data-seat-id')
  expect(targetSeatId).not.toBeNull()
  const targetSeat = page.locator(`[data-seat-id="${targetSeatId}"]`)
  ...
  const downloadedWorkspaceName = download.suggestedFilename()
  expect(downloadedWorkspaceName).toMatch(/\.sce$/i)
  const downloadedWorkspace = await readFile(downloadedWorkspacePath as string)
  ...
  await chooser.setFiles({
    name: downloadedWorkspaceName,
    mimeType: 'application/json',
    buffer: downloadedWorkspace
  })
...
test('keeps the current roster when a malformed workspace is rejected', async ({ page }) => {
  ...
  await expect(page.locator('input[placeholder="未命名"]:visible').first()).toHaveValue('需要保留的学生')
})
