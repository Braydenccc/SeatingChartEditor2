...
export type EditModeValue = typeof EditMode[keyof typeof EditMode]

// 当前编辑模式
const currentMode = ref<EditModeValue>(EditMode.NORMAL)

// 交换模式下第一个选中的座位
const firstSelectedSeat = ref<string | null>(null)

export function useEditMode() {
  // 设置编辑模式
  const setMode = (mode: EditModeValue) => {
  ...
  // 设置第一个选中的座位（交换模式）
  const setFirstSelectedSeat = (seatId: string) => {
  ...
  // 检查是否为指定模式
  const isMode = (mode: EditModeValue) => {
