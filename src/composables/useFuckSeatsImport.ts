...
const isAllowedLocalTarget = (targetUrl: URL): boolean => {
...
const fetchThroughProxy = async (
  ...
  if (!isAllowedLocalTarget(parsedTarget)) {
    window.clearTimeout(timer)
    throw new Error('不想排座位本地地址不在允许范围内')
  }

  try {
    const fetcher = isTauriRuntime()
      ? (await import('@tauri-apps/plugin-http')).fetch
      : globalThis.fetch.bind(globalThis)
    const requestUrl = isTauriRuntime()
      ? targetUrl
      : `${proxyEndpoint}?target=${encodeURIComponent(targetUrl)}`
