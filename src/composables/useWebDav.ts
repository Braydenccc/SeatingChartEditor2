...
export function useWebDav() {
  ...
  const buildDirectUrl = (config: WebDavConfig, path: string) => {
    const baseUrl = normalizeUrl(config.url)
    return path.startsWith('http') ? path : baseUrl + (path.startsWith('/') ? path : '/' + path)
  }

  const buildHeaders = (config: WebDavConfig, options: RequestInit = {}) => {
  ...
  const isNetworkOrCorsError = (error: unknown) => {
    return error instanceof Error && (error.name === 'TypeError' || error.message.includes('Failed to fetch'))
  }
  ...
  const request = async (
    ...
    path: string,
    options: RequestInit = {},
    requestOptions: WebDavRequestOptions = {}
  ): Promise<Response> => {
    ...
    const handleResponse = (response: Response) => {
      ...
      if (response.status === 401) {
        throw new WebDavRequestError('WebDAV 认证失败，请检查用户名和密码', response.status)
      }
      if (response.status === 403) {
        throw new WebDavRequestError('WebDAV 权限不足，请检查账户权限', response.status)
      }
      throw new WebDavRequestError(`WebDAV 请求失败 (${response.status})`, response.status)
    }
  ...
  // 上传文件 (PUT)
  const putFile = async (config: WebDavConfig, path: string, content: BodyInit, contentType = 'text/plain') => {
  ...
  // 下载文件 (GET)
  const getFileText = async (config: WebDavConfig, path: string) => {
  ...
  // 删除文件 (DELETE)
  const deleteFile = async (config: WebDavConfig, path: string) => {
    await request(config, path, { method: 'DELETE' }, { noThrowStatuses: [404] })
    return true
  }

  // 获取目录内容 (PROPFIND)
  const listFiles = async (config: WebDavConfig, path: string): Promise<WebDavFileEntry[]> => {
    const collPath = path.endsWith('/') ? path : path + '/'
