...
describe('useAssignment - Rule Combinations', () => {
  ...
  let zoneData: ReturnType<typeof useZoneData>
  ...
  const getAssignedSeat = (studentId: number | undefined) => {
    const seat = seatChart.seats.value.find(candidate => candidate.studentId === studentId)
    if (!seat) throw new Error(`未找到学生 ${studentId ?? 'unknown'} 的已分配座位`)
    return seat
  }
  ...
  describe('Single Rule Effectiveness', () => {
    ...
    it('should satisfy IN_ROW_RANGE rule', async () => {
      ...
      expect(result.success).toBe(true)
      const student0Seat = getAssignedSeat(studentIds[0])
    ...
    it('should satisfy MUST_BE_SEATMATES rule', async () => {
      ...
      expect(result.success).toBe(true)
      const seat0 = getAssignedSeat(studentIds[0])
      const seat1 = getAssignedSeat(studentIds[1])
  ...
  describe('Multiple Rules Combination', () => {
    ...
    it('should satisfy MUST_BE_SEATMATES + DISTANCE_AT_LEAST combination', async () => {
      ...
      const seat0 = getAssignedSeat(studentIds[0])
      const seat1 = getAssignedSeat(studentIds[1])
      const seat2 = getAssignedSeat(studentIds[2])
      const seat3 = getAssignedSeat(studentIds[3])
