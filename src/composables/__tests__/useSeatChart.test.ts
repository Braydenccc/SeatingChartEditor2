...
describe('useSeatChart', () => {
  ...
  describe('guard seats', () => {
    it('should create guard seats with default enabled config', () => {
      ...
      expect(seatChart.organizedSeats.value.flat(2).some(seat => seat.kind === 'guard')).toBe(false)
    })
    ...
    it('should keep visible guard seats synced after seats are reinitialized', () => {
      expect(requireDefined(seatChart.visibleGuardSeats.value.find(seat => seat.id === 'guard-left')).studentId).toBe(null)

      seatChart.initializeSeats()
      seatChart.assignStudent('guard-left', 1, false)

      expect(seatChart.getStudentAtSeat('guard-left')).toBe(1)
      expect(requireDefined(seatChart.visibleGuardSeats.value.find(seat => seat.id === 'guard-left')).studentId).toBe(1)
    })
  ...
  describe('updateConfig', () => {
    it('should preserve compatible seat state and remove only conflicting assignments', () => {
      ...
      expect(seatChart.getStudentAtSeat('seat-0-0-0')).toBe(1)
      expect(requireDefined(seatChart.getSeat('seat-0-0-1')).isEmpty).toBe(true)
