...
describe('useAssignment - Rule Combination Randomness', () => {
  ...
  describe('Distribution Uniformity with Rules', () => {
    ...
    it('should distribute seatmate pairs across available positions', async () => {
      ...
      const pairPositionCounts: Record<string, number> = {}
      const runs = 100
      ...
      for (let i = 0; i < runs; i++) {
        ...
        const seat0 = requireDefined(seatChart.seats.value.find(s => s.studentId === studentIds[0]))
        const seat1 = requireDefined(seatChart.seats.value.find(s => s.studentId === studentIds[1]))
