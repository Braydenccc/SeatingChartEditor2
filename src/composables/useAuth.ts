...
type LogoutTarget = 'all' | AuthType

const isRecord = (value: unknown): value is Record<string, unknown> =>
    Boolean(value) && typeof value === 'object' && !Array.isArray(value)

const parseWebDavConfig = (value: unknown): WebDavConfig | null => {
    if (!isRecord(value) || typeof value.url !== 'string') return null
    return {
        url: value.url,
        username: typeof value.username === 'string' ? value.username : undefined,
        password: typeof value.password === 'string' ? value.password : undefined,
        encryptedPassword: typeof value.encryptedPassword === 'string' ? value.encryptedPassword : undefined,
        useProxy: value.useProxy === true
    }
}

const parseAuthResult = (value: unknown): AuthApiResult => {
    if (!isRecord(value)) return { success: false, message: '服务器响应格式错误' }
    const rawData = isRecord(value.data) ? value.data : undefined
    const webdav = rawData ? parseWebDavConfig(rawData.webdav) : null
    return {
        success: value.success === true,
        message: typeof value.message === 'string' ? value.message : undefined,
        data: rawData ? {
            username: typeof rawData.username === 'string' ? rawData.username : undefined,
            webdav,
            backupMode: rawData.backupMode === true
        } : undefined
    }
}
...
const authType = ref<AuthType>('retiehe')
...
export const setCookie = (name: string, value: string, days = COOKIE_EXPIRY_DAYS) => {
    let expires = ""
    if (days) {
        const date = new Date()
...
const sanitizeWebdavSettings = (webdav: WebDavConfig | null): WebDavConfig | null => {
    if (!webdav) return null
    const { password, encryptedPassword, authorization, ...safeWebdav } = webdav
    return safeWebdav
}

const decryptWebdavPassword = async (encryptedPassword?: string, legacyKey: string | null = null) => {
...
const preparePersistedWebdavConfig = async (webdav: WebDavConfig | null): Promise<WebDavConfig | null> => {
    if (!webdav) return null

    const safeWebdav = sanitizeWebdavSettings(webdav)
    if (!safeWebdav) return null
    if (webdav.password) {
...
const persistLocalWebdavConfig = (config: WebDavConfig | null) => {
    if (!config) {
...
const verifyRetieheSession = async () => {
    try {
        ...
        const result = parseAuthResult(await response.json().catch(() => null))
...
const initAuth = async () => {
    ...
    if (savedWebdav) {
        try {
            const config = parseWebDavConfig(JSON.parse(savedWebdav))
            if (!config) throw new Error('Invalid WebDAV config')
            ...
            if (!currentUser.value) {
                currentUser.value = { username: config.username || 'WebDAV 用户' }
    ...
    if (savedAuthType && authType.value !== 'webdav') {
        if (savedAuthType === 'retiehe' || savedAuthType === 'webdav') authType.value = savedAuthType
    }
...
const fetchSyncSettings = async (fetchApi: AuthFetch = apiFetch) => {
    ...
    try {
        ...
        const result = parseAuthResult(await response.json())
...
const scheduleFetchSyncSettings = (fetchApi: AuthFetch) => {
...
const attachPasswordField = async (
...
export function useAuth() {
    ...
    const callAuthApi = async (action: 'login' | 'register', username: string, password: string): Promise<AuthApiResult> => {
        ...
        try {
            ...
            return { success: false, message: err instanceof Error ? err.message : '网络请求失败，请检查连接' }
        }
    }
    ...
    const changePassword = async (currentPassword: string, newPassword: string): Promise<AuthApiResult> => {
        cancelScheduledFetchSyncSettings()
        if (!currentUser.value || token.value !== cookieSessionToken) {
    ...
    const logout = async (target: LogoutTarget = 'all') => {
        cancelScheduledFetchSyncSettings()
    ...
    const setWebdavLogin = async (config: WebDavConfig, rememberPassword = false) => {
        authType.value = 'webdav'
    ...
    const updateSyncSettings = async (webdav: WebDavConfig | null, backup: boolean): Promise<AuthApiResult> => {
        cancelScheduledFetchSyncSettings()
