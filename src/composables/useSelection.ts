...
const visitedSeatIds = new Set<string>()
const suppressingContextSelection = ref(false)
let contextSelectionSuppressTimer: ReturnType<typeof setTimeout> | null = null
...
export function useSelection() {
  const addSeatToSelection = (seatId: string) => {
  ...
  const toggleSeatInSelection = (seatId: string) => {
  ...
  const setSelection = (seatIds: Iterable<string>) => {
    selectedSeatIds.value = new Set(seatIds)
  }

  const selectSingleSeat = (seatId: string | null | undefined) => {
    if (!seatId) {
      clearSelection()
      return
    }
  ...
  const startSelection = (seatId?: string) => {
  ...
  const updateSelection = (seatId: string) => {
