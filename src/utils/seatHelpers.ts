 ...
 */
import type { SeatPosition } from '@/types/models'

export function parseSeatId(seatId: string): SeatPosition {
 ...
 */
export function generateSeatId(groupIndex: number, columnIndex: number, rowIndex: number): string {
...
export function generateGuardSeatId(side: 'left' | 'right'): string {
  return side === 'right' ? GUARD_SEAT_RIGHT_ID : GUARD_SEAT_LEFT_ID
}

export function isGuardSeatId(seatId: string): boolean {
