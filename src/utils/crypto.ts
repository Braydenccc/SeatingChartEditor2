 ...
 */
async function deriveKey(keyMaterial: string | null): Promise<CryptoKey> {
 ...
 */
export async function encrypt(password: string, userKey: string | null = null): Promise<string | null> {
    try {
        const encoder = new TextEncoder()
 ...
 */
export async function decrypt(ciphertext: string, userKey: string | null = null): Promise<string | null> {
    try {
        // Base64 解码
...
async function deriveTransportKey(username: string): Promise<CryptoKey> {
...
export async function encryptPasswordForTransport(password: string, username: string): Promise<string | null> {
    const cryptoApi = globalThis.crypto
